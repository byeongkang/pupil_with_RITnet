...
class gaze_inferer(object):
    def __init__(self, model, flen, ori_video_shape, sensor_size, infer_gaze_flag=True):
        ...
        # self.eyefitter = SingleEyeFitter(focal_length=self.flen,
        #                                  pupil_radius=2 ,
        #                                  initial_eye_z=50 )
    ...
    def process(self, video_src, mode, output_record_path="", batch_size=32,
                output_video_path="", heatmap=False, print_prefix="",fit_frame_count=60) -> object:
        """
        ...
        self.vid_manager = VideoManager(vreader,output_record_path=output_record_path,
        ...
        self.eyefitter.focal_length = self.flen * self.mm2px_scaling * image_scaling_factor #mm --> pixel size
        self.eyefitter.pupil_radius = 2 * self.mm2px_scaling * image_scaling_factor  #mm --> pixel size
        ...
        for idx, frame in enumerate(vreader.nextFrame()):
            ...
            frame_filename = f"./frame/{idx}.png"
            ...
            # After reaching the batch size, but not the final batch, predict heatmap and fit/infer angles
                ...
                if mode == "Fit_and_Infer":
                    if idx < fit_frame_count:  # fit_frame_count까지 Fit 모드 수행
                        self._fitting_batch(X_batch=X_batch, Y_batch=Y_batch)
                    else:  # 이후 프레임들은 Infer 모드로 전환
                        _, _, _ = self._infer_batch(X_batch=X_batch, Y_batch=Y_batch, idx=idx - final_batch_size)
    ...
    def _fitting_batch(self, X_batch, Y_batch):
        ...
        for batch_idx, (X_each, Y_each) in enumerate(zip(X_batch, Y_batch)):
            pred_each = Y_each[:, :, 1] #mask-confidence
    ...
    def _infer_batch(self, X_batch, Y_batch, idx):
        ...
        for batch_idx, (X_each, Y_each) in enumerate(zip(X_batch, Y_batch)):
            ...
            if (centre is not None) and self.infer_gaze_flag:
                ...
                # Eyeball center from self.eyefitter.eye_centre, which is a (3, 1) ndarray
                eyeball_center_x, eyeball_center_y, eyeball_center_z = self.eyefitter.eye_centre[0, 0], self.eyefitter.eye_centre[1, 0], self.eyefitter.eye_centre[2, 0]

                # Extract gaze normal vector from n1, which is a 2D numpy array (3, 1)
                gaze_normal_x, gaze_normal_y, gaze_normal_z = n1[0, 0], n1[1, 0], n1[2, 0]




                #self.eyefitter.eye_centre[1, 0], self.eyefitter.eye_centre[2, 0]
                self.vid_manager.write_results(frame_id=frame, eyeball_center_x = eyeball_center_x, eyeball_center_y = eyeball_center_y,eyeball_center_z = eyeball_center_z,
                                               gaze_normal_x=n1[0,0], gaze_normal_y=n1[1,0],gaze_normal_z=n1[2,0])
            ...
            # If ellipse fitting is successful, i.e. an ellipse is located, AND gaze inference is DISABLED
                ...
                self.vid_manager.write_results(frame_id=frame, eyeball_center_x=eyeball_center_x,
                                               eyeball_center_y=eyeball_center_y, eyeball_center_z=eyeball_center_z,
                                               gaze_normal_x=n1[0, 0], gaze_normal_y=n1[1, 0], gaze_normal_z=n1[2, 0])
            ...
            # IF ellipse fitting is unsuccessful.
                ...
                positions, gaze_angles, inference_confidence = None, None, None
                eyeball_center_x, eyeball_center_y, eyeball_center_z = 0,0,0
                ...
                self.vid_manager.write_results(frame_id=frame, eyeball_center_x=eyeball_center_x,
                                               eyeball_center_y=eyeball_center_y, eyeball_center_z=eyeball_center_z,
                                               gaze_normal_x=0, gaze_normal_y=0, gaze_normal_z=0)
