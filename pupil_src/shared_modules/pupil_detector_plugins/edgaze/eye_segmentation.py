...
class EyeSegmentation(object):
    # print("현재 실행 중인 파일의 디렉토리:", os.path.dirname(os.path.abspath(__file__)))
    ...
    def predict(self, framename):
        ...
        print(f"Segmentation time: {inference_time:.4f} seconds")
    ...
    def predict_image(self, img):
        """Predict eye segmentation from an in-memory image array."""
        ...
        # 이미지 전처리
        img_processed = self.get_img(img)

        # 텐서로 변환 후 배치 차원 추가
        data = torch.tensor(img_processed, device=self.device).float().unsqueeze(0)
